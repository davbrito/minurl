...
export async function loader({ params, context, request }: Route.LoaderArgs) {
  ...
  const analytics = await getAnalyticsForLink(context, params.id, 25);
  const baseUrl = new URL(request.url).origin;
  const shortUrl = `${baseUrl}${getMinifiedPath(params.id)}`;
  return { id: params.id, data, shortUrl, analytics };
}
...
export default function Minified({ loaderData }: Route.ComponentProps) {
  const { id, data, shortUrl, analytics } = loaderData;
  return (
    <div className="grow p-6">
      <Preview id={id} data={data} shortUrl={shortUrl} analytics={analytics} />
    </div>
  );
}
